...
def get_api_keys():
    return {
        ...
        'huggingface': st.secrets.get('HUGGINGFACE_API_KEY', ''),
...
def generate_image(prompt, api_key):
    try:
        ...
        if response.status_code == 503:
            return None, "⏳ Model is loading, please try again in 30 seconds..."
...
st.markdown("Compare responses from **Claude**, **Mistral**, and **Perplexity** • Generate images with AI • Save your searches")
st.markdown("---")
...
# Main content
tab1, tab2 = st.tabs(["📝 Text Search", "🖼️ Image Generation"])
        ...
        with col2:
            if 'mistral' in st.session_state.responses:
                ...
                if st.button("📋 Copy Mistral", key="copy_mistral"):
                    st.write("```")
                    st.write(st.session_state.responses["mistral"])
                    st.write("```")
        ...
        with col3:
            if 'perplexity' in st.session_state.responses:
                ...
                if st.button("📋 Copy Perplexity", key="copy_perplexity"):
                    st.write("```")
                    st.write(st.session_state.responses["perplexity"])
                    st.write("```")
...
with tab2:
    ...
    image_prompt = st.text_area(
        ...
        placeholder="e.g., 'A futuristic city with flying cars at sunset', 'A cat wearing sunglasses', etc.",
        height=80
    )
    ...
    if generate_button and image_prompt:
        if not api_keys['huggingface']:
            ...
            with st.spinner("🎨 Generating your image..."):
                image_data, error = generate_image(image_prompt, api_keys['huggingface'])
    ...
    if st.session_state.generated_image:
        ...
        st.info(f"📝 Prompt: {image_prompt}")

# Footer
st.markdown("---")
st.markdown("""
<div style="text-align: center; color: #888;">
    <p>💡 Free Multi-AI Search Engine | Compare Claude, Mistral & Perplexity | Generate Images with Stable Diffusion</p>
